...
public class InitInstancedAnimation implements LifeCycle {
                    ...
                    for(int x = -count; x < count; x++) {
                        for(int y = -count; y < count; y++) {
                            for(int z = -count; z < count; z++) {
                                ...
                                cluster.add(new Instance(trafo, MaterialFactory.getInstance().getMaterialsAsList().get((x+y+z+3*count)%10), new AnimationController(120,24 + randomFloat*7), new SimpleSpatial(){
                                    @Override
