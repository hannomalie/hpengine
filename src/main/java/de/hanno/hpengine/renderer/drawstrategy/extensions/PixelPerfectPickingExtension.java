...
public class PixelPerfectPickingExtension implements RenderExtension {
    ...
    public void renderFirstPass(FirstPassResult firstPassResult, RenderState renderState) {
        ...
        if (Input.PICKING_CLICK == 1) {
            ...
            floatBuffer.rewind();
            Vector2f ratio = new Vector2f(Float.valueOf(Engine.getInstance().getConfig().getWidth()) / Float.valueOf(ApplicationFrame.WINDOW_WIDTH),
                    Float.valueOf(Engine.getInstance().getConfig().getHeight()) / Float.valueOf(ApplicationFrame.WINDOW_HEIGHT));
