...
public class RenderProbeCommand implements Command<Result>, Transformable {
	...
	public Result execute(AppContext appContext) {
		...
		probe.draw(urgent, new RenderExtract().init(appContext.getActiveCamera(), appContext.getScene().getEntities(), appContext.getScene().getDirectionalLight(),true,true,true,false, appContext.getScene().getMinMax()[0], appContext.getScene().getMinMax()[1]));
