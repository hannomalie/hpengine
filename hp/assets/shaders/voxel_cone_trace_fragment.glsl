...
void main(void) {
    ...
    if(!debugVoxels && useVoxelConeTracing &&
		...
		vec4 voxelSpecular = voxelTraceCone(grid, gridSize, sceneScale, 2*sceneScale, positionWorld+sceneScale*normalWorld, normalize(reflect(-V, normalWorld)), 0.1*roughness, 370); // 0.05
