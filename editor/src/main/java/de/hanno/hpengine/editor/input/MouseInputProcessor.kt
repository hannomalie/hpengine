...
class MouseInputProcessor(
    ...
    override fun mouseDragged(e: MouseEvent) =
        addResourceContext.launch { // TODO: This is not too nice but I don't know how else it could be done
            ...
            if (entityOrNull == null) {
                ...
                activeCamera.viewProjectionMatrix.project(
                    entityOrNull.transform.position,
                    viewPort,
                    entityPositionDevice
                )
                ...
                val constraintAxis = editorConfig.constraintAxis
                ...
                fun handleTranslation() {
                    when (editorConfig.transformSpace) {
                        TransformSpace.World -> entityOrNull.transform.set(
                            Matrix4f(oldTransform).translateLocal(
                                axisConstrainedMovement
                            )
                        )
                        TransformSpace.Local -> entityOrNull.transform.set(
                            Matrix4f(oldTransform).translate(
                                axisConstrainedMovement
                            )
                        )
                        TransformSpace.View -> entityOrNull.transform.set(
                            Matrix4f(oldTransform).translate(
                                activeCamera.getViewDirection().mul(axisConstrainedMovement)
                            )
                        )
                    }
                }
                ...
                fun handleRotation() {
                    ...
                    val pivot = pivot.position

                    when (editorConfig.rotateAround) {
                        RotateAround.Self -> when (editorConfig.transformSpace) {
                            TransformSpace.World -> when (constraintAxis) {
                                AxisConstraint.X -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateAffine(degreesX, -1f, 0f, 0f))
                                AxisConstraint.Y -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateAffine(degreesY, 0f, 1f, 0f))
                                AxisConstraint.Z -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateAffine(degreesY, 0f, 0f, -1f))
                                AxisConstraint.None -> Unit
                            }
                            TransformSpace.Local -> when (constraintAxis) {
                                AxisConstraint.X -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateX(degreesX))
                                AxisConstraint.Y -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateY(degreesY))
                                AxisConstraint.Z -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateZ(degreesY))
                                AxisConstraint.None -> Unit
                            }
                            TransformSpace.View -> when (constraintAxis) {
                                AxisConstraint.X -> entityOrNull.transform.set(oldTransform).mul(
                                    Matrix4f().identity()
                                        .rotateAffine(degreesX, cameraRight.x, cameraRight.y, cameraRight.z)
                                )
                                AxisConstraint.Y -> entityOrNull.transform.set(oldTransform).mul(
                                    Matrix4f().identity().rotateAffine(degreesY, cameraUp.x, cameraUp.y, cameraUp.z)
                                )
                                AxisConstraint.Z -> entityOrNull.transform.set(oldTransform).mul(
                                    Matrix4f().identity()
                                        .rotateAffine(degreesY, cameraView.x, cameraView.y, cameraView.z)
                                )
                                AxisConstraint.None -> Unit
                            }
                        }
                        RotateAround.Pivot -> when (editorConfig.transformSpace) {
                            TransformSpace.World -> when (constraintAxis) {
                                AxisConstraint.X -> entityOrNull.transform.set(
                                    Matrix4f(oldTransform).rotateAroundLocal(
                                        AxisAngle4f(degreesX, -1f, 0f, 0f),
                                        pivot
                                    )
                                )
                                AxisConstraint.Y -> entityOrNull.transform.set(
                                    Matrix4f(oldTransform).rotateAroundLocal(
                                        AxisAngle4f(degreesY, 0f, 1f, 0f),
                                        pivot
                                    )
                                )
                                AxisConstraint.Z -> entityOrNull.transform.set(
                                    Matrix4f(oldTransform).rotateAroundLocal(
                                        AxisAngle4f(degreesY, 0f, 0f, -1f),
                                        pivot
                                    )
                                )
                                AxisConstraint.None -> Unit
                            ...
                            TransformSpace.Local -> when (constraintAxis) {
                                AxisConstraint.X -> entityOrNull.transform.set(
                                    Matrix4f(oldTransform).rotateAroundLocal(
                                        AxisAngle4f(degreesX, entityRight),
                                        pivot
                                    )
                                )
                                AxisConstraint.Y -> entityOrNull.transform.set(
                                    Matrix4f(oldTransform).rotateAroundLocal(
                                        AxisAngle4f(degreesY, entityUp),
                                        pivot
                                    )
                                )
                                AxisConstraint.Z -> entityOrNull.transform.set(
                                    Matrix4f(oldTransform).rotateAroundLocal(
                                        AxisAngle4f(degreesY, entityView),
                                        pivot
                                    )
                                )
                                AxisConstraint.None -> Unit
                            ...
                            TransformSpace.View -> when (constraintAxis) {
                                AxisConstraint.X -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateAffine(degreesX, cameraRight))
                                AxisConstraint.Y -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateAffine(degreesY, cameraUp))
                                AxisConstraint.Z -> entityOrNull.transform.set(oldTransform)
                                    .mul(Matrix4f().identity().rotateAffine(degreesY, cameraView))
                                AxisConstraint.None -> Unit
                            }
                        }
                    }.let { }
                ...
                fun handleScaling() {
                    ...
                    when (editorConfig.transformSpace) {
                        TransformSpace.World -> when (constraintAxis) {
                            AxisConstraint.X -> entityOrNull.transform.set(oldTransform)
                                .scaleAroundLocal(axisConstrainedScaleAmount, entityOrNull.transform.position)
                            AxisConstraint.Y -> entityOrNull.transform.set(oldTransform)
                                .scaleAroundLocal(axisConstrainedScaleAmount, entityOrNull.transform.position)
                            AxisConstraint.Z -> entityOrNull.transform.set(oldTransform)
                                .scaleAroundLocal(axisConstrainedScaleAmount, entityOrNull.transform.position)
                            AxisConstraint.None -> entityOrNull.transform.set(oldTransform)
                                .scaleAroundLocal(axisConstrainedScaleAmount, entityOrNull.transform.position)
                        }.let { }
                        TransformSpace.Local -> when (constraintAxis) {
                            AxisConstraint.X -> entityOrNull.transform.set(oldTransform).scaleAroundLocal(
                                axisConstrainedScaleAmount.rotate(entityOrNull.transform.rotation),
                                entityOrNull.transform.position
                            )
                            AxisConstraint.Y -> entityOrNull.transform.set(oldTransform).scaleAroundLocal(
                                axisConstrainedScaleAmount.rotate(entityOrNull.transform.rotation),
                                entityOrNull.transform.position
                            )
                            AxisConstraint.Z -> entityOrNull.transform.set(oldTransform).scaleAroundLocal(
                                axisConstrainedScaleAmount.rotate(entityOrNull.transform.rotation),
                                entityOrNull.transform.position
                            )
                            AxisConstraint.None -> entityOrNull.transform.set(oldTransform)
                        ...
                        TransformSpace.View -> when (constraintAxis) {
                            AxisConstraint.X -> entityOrNull.transform.set(oldTransform).scaleAroundLocal(
                                axisConstrainedScaleAmount.rotate(activeCamera.entity.transform.rotation),
                                entityOrNull.transform.position
                            )
                            AxisConstraint.Y -> entityOrNull.transform.set(oldTransform).scaleAroundLocal(
                                axisConstrainedScaleAmount.rotate(activeCamera.entity.transform.rotation),
                            ...
                            AxisConstraint.None -> entityOrNull.transform.set(oldTransform).scaleAroundLocal(
                                axisConstrainedScaleAmount.rotate(activeCamera.entity.transform.rotation),
                                entityOrNull.transform.position
                            )
                        }.let { }
                ...
                when (editorConfig.transformMode) {
...
fun Matrix4f.scaleAroundLocal(factor: Vector3f, o: Vector3f) =
    scaleAroundLocal(factor.x, factor.y, factor.z, o.x, o.y, o.z)

fun Matrix4f.scaleAroundLocal(factor: Vector3f, ox: Float, oy: Float, oz: Float) =
    scaleAroundLocal(factor.x, factor.y, factor.z, ox, oy, oz)

fun Matrix4f.rotateAroundLocal(axisAngle: AxisAngle4f, o: Vector3f): Matrix4f =
    rotateAroundLocal(Quaternionf(axisAngle), o.x, o.y, o.z)
