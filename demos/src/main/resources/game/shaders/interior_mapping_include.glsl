...
newV = newV;//(rotationMatrix * vec4(newV, 0)).xyz;
...
if(isWindow) {
    ...
    if(material.worldSpaceTexCoords == 2) {
        out_color.rgba = vec4(textureLod(normalMap, position_world.zy * 0.05, 0).rgb, 1);
    } else {
        out_color.rgba = vec4(textureLod(normalMap, position_world.xy * 0.05, 0).rgb, 1);
    }
}