        ...
        forEachEntity { parentEntityId ->
            ...
            if (preventDefaultRenderingComponentMapper[entityId] == null) {
                if (modelComponent != null && modelCacheComponent != null) {
                    ...
                    val entityIndexOf =
                        entityBuffer.getEntityIndex(parentEntityId)!! // TODO: Factor in instance index here

                    val model: Model<*> = modelCacheComponent.model
                    val meshes = model.meshes
                    for (meshIndex in meshes.indices) {
                        val mesh = meshes[meshIndex]
                        val aabb = model.getBoundingVolume(
                            transform,
                            mesh
                        )

                        val visibleForCamera =
                            camera.contains(aabb) || instanceCount > 1 // TODO: Better culling for instances
