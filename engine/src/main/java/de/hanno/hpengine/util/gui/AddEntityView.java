...
public class AddEntityView extends WebPanel {
	...
	public AddEntityView(Engine engine, WebFrame addEntityFrame, HostComponent hostComponent) {
		this.engine = engine;
	...
	private void addOkButton(List<Component> panels) {
		...
		saveButton.addActionListener(e -> {
			...
			try {
				chosenFiles = WebFileChooser.showMultiOpenDialog("./hp/assets/models/", customizer);
			} catch (Exception ex) {
				chosenFiles = WebFileChooser.showMultiOpenDialog();
			}
			if(chosenFiles != null) {
