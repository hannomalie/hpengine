        ...
        if (Config.getInstance().isPrintPipelineDebugOutput) {
            ...
            fun printPhase1(commandOrganization: CommandOrganization) {
                with(commandOrganization) {
                    ...
                    Util.printIntBuffer(visibilityBuffers[phase]!!.buffer.asIntBuffer(), if (commands.isEmpty()) 0 else commands.map { it.primCount }.reduce({ a, b -> a + b }), 1)
                    println("Entity instance counts")
                    Util.printIntBuffer(entitiesCounters[phase]!!.buffer.asIntBuffer(), commands.size, 1)
                    ...
                    Util.printIntBuffer(entityOffsetBuffersCulled[phase]!!.buffer.asIntBuffer(), commands.size, 1)
                    println("CurrentCompactedPointers")
                    Util.printIntBuffer(currentCompactedPointers[phase]!!.buffer.asIntBuffer(), commands.size, 1)
                    println("CommandOffsets")
                    Util.printIntBuffer(commandOffsets[phase]!!.buffer.asIntBuffer(), commands.size, 1)
                    if(!commands.isEmpty()) {
                        println("Commands")
                        Util.printIntBuffer(commandBuffers[phase]!!.buffer.asIntBuffer(), 5, drawCountBuffers[phase]!!.buffer.asIntBuffer().get(0))
