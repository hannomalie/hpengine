...
class CombinePassRenderExtension(val engineContext: EngineContext): RenderExtension<OpenGl> {
    ...
    private val combineProgram = engineContext.programManager.getProgram(
        engineContext.config.EngineAsset("shaders/combine_pass_vertex.glsl").toCodeSource(),
        engineContext.config.EngineAsset("shaders/combine_pass_fragment.glsl").toCodeSource(),
        Uniforms.Empty
    )
