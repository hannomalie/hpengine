...
//				cubeMapProgram.use();
    ...
    fun draw(urgent: Boolean = false) {
        ...
        val dynamicProbes = probes.stream().filter { probe: EnvironmentProbe -> probe.probeUpdate === EnvironmentProbe.Update.DYNAMIC }.collect(Collectors.toList())
    ...
    override suspend fun update(scene: Scene, deltaSeconds: Float) {
        probes.forEach { it.update(scene, deltaSeconds) }
